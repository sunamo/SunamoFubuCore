...
public class LocalTime : IComparable<LocalTime>
{
    ...
    public DateTime Time => UtcTime.ToLocalTime(TimeZone);
    ...
    public static LocalTime GuessDayFromTimeOfDay(LocalTime currentTime, TimeSpan timeOfDay)
    {
        ...
        if (Math.Abs(currentTime.Subtract(today).TotalHours) < 12) return today;

        var yesterday = today.Add(-1.Days());
        if (Math.Abs(currentTime.Subtract(yesterday).TotalHours) < 12) return yesterday;
