...
public class FileSystem : IFileSystem
{
    ...
    public async Task WriteStringToFile(string filename, string text)
    {
        CreateDirectory(Path.GetDirectoryName(filename));
...
TFSE.ReadAllText(filename);
    ...
    public void AlterFlatFile(string path, Action<List<string>> alteration)
    {
        ...
        if (FileExists(path)) ReadTextFile(path, list.Add);
    ...
    {
        if (Directory.Exists(directory)) return Directory.GetDirectories(directory);
    ...
    public void ReadTextFile(string path, Action<string> callback)
    {
        ...
        {
            string line;
            while ((line = reader.ReadLine()) != null) callback(line.Trim());
        }
    }
    ...
    private void internalFileCopy(string source, string destination, CopyBehavior behavior)
    {
        ...
        if (isFile) destinationDir = Path.GetDirectoryName(fullDestPath);
