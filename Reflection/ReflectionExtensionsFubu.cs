...
public static class ReflectionExtensionsFubu
{
    ...
    {
        foreach (T attribute in provider.GetCustomAttributes(typeof(T), true)) action(attribute);
    }
    ...
    {
        foreach (T attribute in accessor.InnerProperty.GetCustomAttributes(typeof(T), true)) action(attribute);
    }
