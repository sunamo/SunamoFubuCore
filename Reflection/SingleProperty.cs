...
public class SingleProperty : Accessor
{
    ...
    {
        get { return new[] { InnerProperty.Name }; }
    }
    ...
    public Expression<Func<T, object>> ToExpression<T>()
    {
        ...
        if (InnerProperty.PropertyType.IsValueType) body = Expression.Convert(body, typeof(object));
    ...
    {
        if (InnerProperty.CanWrite) InnerProperty.SetValue(target, propertyValue, null);
    }
